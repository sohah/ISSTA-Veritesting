...
\begin{algorithm}[]
\SetAlgoLined
Input: (symbolic if-instruction inst)\;
$\region_{initial}$ = jit-region-construction(inst)\;
$\region_{no\_return}$ =  early-return transformation($\region_{initial}$)\;
...
\eIf{is-canonical($\region_{green}$)}
   ...
   {abort \tcp*{resume DSE of if-instruction}}  
   
\caption{Ranger general pseudocode}
\label{fig:algorithm}
\end{algorithm}

\newtext{Algorithm~\ref{fig:algorithm} describes how \tool\ works. The main idea is that \toolshort\  intercepts symbolic if-instructions and tries to capture the 
summary of instructions that lie along its internal control flows and up until the phi instruction. We call this summarization a {\bf Region} or just \region. Now it is important to note that, \region\ does not only contain the summarization in form of some \toolshort\ IR statement, it also contains other environment-related information, such as types, input and outputs in the region, explained later in this section.}

\newtext{To do that, \tool\ uses just-in-time analysis (jit analysis) and with the help of Wala~\cite{Wala} as a static analysis tool, \toolshort\ constructs the cfg of the method embedding the intercepted symbolic if instruction. Then \toolshort\ translates the part of the cfg corresponding to the if-branches until the immediate common successor, into \tool\  IR statement and some region related environment information (line 2). We call this step \bf{jit-region construction}.}

\newtext{\tool\ then runs different transformations on \region\ (lines 2-22) with the goal of reducing the region IR statement into some canonical form that can be easily translated to the solver. }
\newtext{Because the dependency of running some transformations, can be bidirectional. For example, inlining methods could result in some field statements that requires the field-references-transformation to run on, but likewise, field-transformation could result in propagating information about a method that should be inlined through the high-order-transformation. Using the two fixed points loop, \toolshort\ ensures that such dependencies are taken into consideration.}

\newtext{If the transformations successfully produces a canonical form of the region, then \tool\ adds the summarization to the path condition, populate other symbolic objects and sets the execution point of DSE to start on the instruction following the summarized if-instruction. Otherwise, \toolshort\ aborts and allows DSE to resume execution (lines 23-28).}
%
%%
...
\\
\text{Early-return-next-instruction} & Next_{ret}: & \mathbb{Z} &  & 
\end{array}
$$
...
\newtext{Java Ranger defines 4 types of variables, a field or array variable (created to maintain fields and arrays ssa), an
...
\label{sec:jit}
\newtext{As mentioned before the construction of the region happens when \tool\ detects a symbolic if-instruction during DSE, and thus an opportunity for path-merging. Using the recovered cfg of the method where the intercepted if-instruction resides, \toolshort\ constructs two types of regions: \textit{multi-path region} and \textit{method-region}.} 

\newtext{A \textit{multi-path region}: contains a statement in \toolshort\ IR and a coressponding environment. The IR statement represents the summarization of the instructions from the conditional instruction basic block, i.e, if-bytecode basic block, and down until the merge point, i.e, the phi basic block. The region environment that contains information about variables in the region, such as types of variables, variables stack slot information, inputs of the region and output of the region.}

\newtext{A \textit{method-region}: also contains a statement in \toolshort\ IR and an environment. This time however the IR statement represents the summarization of the whole method. The environment for a method-region is almost similar to the multi-path region, except that the input and the stack slot of variables are defined in terms of the parameters to the method.}  

\newtext{In the next subsections we describe the two main steps in constructing a region, the IR statement creation, which we call \textit{IR statement recovery} and the environment creation for regions.}
...
\newtext{\textbf{Early-next-instruction}: is a the instruction position that DSE should resume from, if path-merging was successful for early return case.}
...
\infer[\rn{high-order_1}]
 ...
 {\step{\Gamma_r, \Delta_r}{e_1}{\Gamma_r, \Delta_r'}{v_1}{sub} \qquad \lookup{\Gamma_s}{v_1} = \tau , \qquad \Gamma_r', \lookup{\Psi}{\tau}=\lambda x.s_2, \qquad \step{\Delta_r'}{e_2}{\Delta_r''}{v_2}{sub}}
$$
$$
...
%
\newtext{The $\rn{substitution}$ rule in Figure~\ref{fig:semantics} describes the semantics of this transformation where the location of an input variable $l$ is used to lookup its value in the DSE environment $\Delta_s$ and is substituted in the region statement.}
%
