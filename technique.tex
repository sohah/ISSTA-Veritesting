...
\begin{itemize}
\item {Renaming Transformation}: In Alpha renaming transformation, all Ranger IR variables are renamed to ensure their uniqueness before further processing takes place. 
%
...
%
\item{Local Variable Substitution Transformation}: During this transformation we eagerly bring in all dynamically known constant values, symbolic values and references into the region for further processing. \\
%
\item{Higher-order Regions Transformation}: This transformation happens as a part of the substitution transformation.
%
...
\\
\item{Field References SSA form}: The field references transformation translates reads and writes of fields
...
\item{Array References SSA form}: The array references transformation translates reads and writes of arrays in
...
\item{Type Propagation}: Ranger IR needs to have type information for its variables so that it can construct
...
%
\item{Simplification of Ranger IR}: The Ranger IR constructed by earlier transformations computes exact semantics
...
\item{Single Path Cases}: This transformation collects path predicates inside the Ranger region that lead to \textit{non-nominal} exit point.
%
