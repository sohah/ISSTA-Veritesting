...
\alt <var> '=' <exp> | 'new' 'type'
\alt <invokeStmt> <class-type> <method-type>
\alt <var> '=' <invokeStmt> <class-type> <method-type>
\alt 'throw' 'exception' | 'skip' | 'return'

<invokeStmt> ::= 'invokeVirtual' | 'invokeStatic' | 'invokeSpecial' | 'invokeInterface'

<exp> ::= <var> | <exp> $\diamond_b$ <exp> | $\diamond_u$ <exp> | <gamma-exp>

<gamma-exp> ::= <exp> $\diamond_b$ <exp> '?' <exp> ':' <exp>

<var> ::= <local-var> | <field-var> | <array-var> | <intermediate-var> | <constant>

<local-var> ::= <intermediate-var> '_' <stack-slot>

<field-var> ::= 'f' <var> <class-type> <field-name> <path-subscript> <global-subscript>

<array-var> ::= 'a' <var> <var> <path-subscript> <global-subscript>

<intermediate-var> ::= 'w' <id>

<$\diamond_b$> ::= $+$ | $-$ | $*$ | $\div$ | \& | bitwise-or | $\oplus$ | \% | $==$ | $\neq$ | $\leq$ | $\geq$ | \&\& | $||$ | \textgreater | \textless | $\ll$ | $\gg$ | $\ggg$

<$\diamond_u$> ::= $-$ | \textasciitilde

%<class-type> ::= canonical class name
%
%<method-type> ::= JVM-internal method signature
%
%<stack-slot> ::= integer stack slot value for <local-var>
%
%<field-name> ::= name of a field within a class
%
%<path-subscript> ::= integer subscript incremented for each field access in a path
%
%<global-subscript> ::= integer subscript incremented for each field access in a region
%
%<constant> ::= integer or real constant value
\end{grammar}
%
