...
Input: Region R\;
 ...
 \Repeat{!(somethingChanged)}
 {
	\While{(somethingChanged)}{
		...
		execute higher-order method inlining transformation(R)\;
...
\eIf{R successfully transformed}{populate outputs\;}{abort\;}
...
%
For the motivating example in Figure~\ref{fig:mot-example}, the recovered statement is as follows:
%
\begin{lstlisting}
if ((! (== x58 0 ) )) {
  x44 = invokeinterface < Application, Ljava/util/List, get(I)Ljava/lang/Object; >[x9,x59]
  x45 = checkcast <Application,Ljava/lang/Integer>x44 <Application,Ljava/lang/Integer>
  x47 = invokevirtual < Application, Ljava/lang/Integer, intValue()I > x45
  if ((! (!= x47 0 ) )) {
    x48 := (+ x57 1 );
  }
} else {
...
%In the following sections we will discuss the various parts of this statement to show the effect of various transformations.
%
We present the grammar that described Ranger IR in Figure~\ref{fig:grammar}.
%
...
D_$\mathbb{N}$), binary and unary operations over expressions and reads and writes from fields and arrays, and
a special gamma expression, $gamma(exp, exp, exp)$, that is the ternary operator.
%
Gamma expressions allow Ranger IR to construct Gated Single Assignment~(GSA) form for variables.
%
Java Ranger defines 3 types of variables, a field or array variable (created to maintain fields and arrays ssa), an
early return variable (to carry early return values), variables that correspond to local variables, and intermediate variables.
...
%
For example, the local variable inputs are Ranger IR variables in the region summary the make the first \textit{use} of a
stack slot.
%
...
array references.
%
We also use the instantiation environment to map Ranger IR variables to their corresponding stack slots, starting with
...
\infer[\rn{high-order_1}]
 {\step{\Gamma_r, \Delta_r}{s_1; invoke(\tau, e_1, e_2)}{(\Gamma_r \cup \Gamma_r'), \Delta_r''}{s_1; \sub{v_2}{x}{s_1}}{high}}
...
\infer[\rn{high-order_2}]
 {\step{\Gamma_r, \Delta_r}{e = invoke(\tau, e_1, e_2)}{(\Gamma_r \cup \Gamma_r'), \Delta_r''}{\sub{v_2}{x}{s} ; e = e'}{high}}
...
\infer[\rn{get-return_2}]
 ...
 {\step{\Sigma}{s_1}{\Sigma_{ret}: (w1_{ret},c_1)}{s_1' ; w1_{ret} := e_1''; \return e_1'}{}  \qquad
  \step{\Sigma}{s_2}{\Sigma_{ret}: (w2_{ret}, c_2)}{s_2' ; (w2_{ret} := e_2'' ; \return e_2')}{} \qquad gen\_id() = x_{ret}}
$$
$$
\infer[\rn{get-field_1}]
 {\step{\Gamma}{x := get\text{-}field(e)}{\Gamma'}{ x := (v,e')}{field}}
 {\step{\Gamma, psm}{e}{\Gamma'}{v}{field} \quad \Theta(v) = l \qquad \Delta_s (v )=(v,e') \qquad psm(v) = \phi}
$$
$$
...
uniqueness before further processing takes place.
%
...
To give an example of this transformation from our motivating example, line 2 gets changed to:
\begin{lstlisting}
    x44\$2 = invoke < Application, Ljava/util/List, get(I)Ljava/lang/Object; >[w9$2, w59$2]
\end{lstlisting}
...
To continue the example used in the alpha-renaming transformation, the reference of the arraylist can be
...
%
(1) The called method\rq s summary is retrieved and alpha renaming of Ranger IR variables is applied on it.
%
...
read as a local variable input that had the symbolic value $x40\$1$ (a consequence of summarizing in lines 8--10 in
...
yields the following Ranger IR statement.
\begin{lstlisting}
if ((! (== x40$1 0 ) )) {
  x4$4 := 200;
  if ((! (< 0 x4$4 ) )) {
...
substituted by its constant value {\tt 1}.
%
\begin{lstlisting}
if ((!= x40$1 0 )) {
...
x55$2 := (Gamma x40$1!=0 (Gamma x47$2==0 0 x40$1) (Gamma x53$2!=0 1 x40$1));
\end{lstlisting}
\textbf{Array References SSA Transformation}: The array references transformation translates reads and writes of arrays in
...
%
The effect of this transformation on the last shown Ranger IR statement produces the following statement.
%
For this region instantiation, the value present at index 0 in array reference 397 was 380.
\begin{lstlisting}
if ((!= x40$1 0 )) {
  if ((&& (< 0 200 ) (>= 0 0 ) )) {
    x5$5 := 380;
...
folding~\cite{dragon-book} to avoid unnecessary formulas in the region by representing constant assignments and copies
in the region\rq s instantiation environment.
%
This transformation also simplifies away if-then-else expressions and if-then-else statements where both choices are
syntactically equal.
%
...
\textit{non-nominal} exit point as an alternative to transition points as presented by Avgerinos et al~\cite{veritesting}.
%
...
Running this transformation on the last shown Ranger IR statement (after another simplification) produces the following statement.
%
Please note that the {\tt value} field in the object referenced by the value 380 was set to the symbolic integer {\tt x1}.
\begin{lstlisting}
