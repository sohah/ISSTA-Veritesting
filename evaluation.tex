...
%
\begin{enumerate}
\item In mode 1, \tool\ runs vanilla SPF without any path merging enabled.
%
\item In mode 2, \tool\ summarizes multi-path regions with a single exit point and instantiates them if they are encountered.
...
%
\item In mode 3, in addition to using summaries instantiated in mode 2, \tool\ instantiates all multi-path region
...
%
\item In mode 4, in addition to using summaries instantiated in mode 3, \tool\ uses single-path cases to allow
multi-path regions to have more than one exit point.
%
These exit points take the form of new object allocations, exceptional behavior present in the region, and method
invocations for which no summaries are present.\vaibhav{Soha to double-check this statement}
%
\item In mode 5, in addition to using summaries instantiated in mode 4, \tool\ converts multiple exit points that return
...
\begin{table*}[]
    \begin{tabular}{@{}ccccccccc@{}}
        \toprule
        WBS  & TCAS & replace & NanoXML & Siena & Schedule2 & PrintTokens2 & ApacheCLI & MerArbiter \\ \midrule
        1234 & 4509 & 3980    & 871     & 1551  & 722       & 1180         & 910       & 20639      \\ \bottomrule
    \end{tabular}
...
\begin{figure}%
    ...
    \caption{Performance of \tool\ on ApacheCLI without and with the last input made symbolic, and on MerArbiter}%
    \label{fig:results2}%
\end{figure}
...
On the ApacheCLI benchmark, while the most significant benefit is introduced in mode 2 of \tool, the benefit in terms
of execution path count and running time is still not lost in modes 3, 4, and 5.
%
...
%
Since \tool\ can summarize multi-path regions with stack outputs, it summarizes and instantiates all such regions
(about 464,000 instantiations with 7 steps of MerArbiter) and gives symbolic exploration of MerArbiter an exponential speedup.
